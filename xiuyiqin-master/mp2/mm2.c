...
void mm2(double *restrict A, const double *restrict B, const double *restrict C, size_t N) { 
        int rank; 
        int size; 
        ...
        istart = f1(rank, size, N); 
        iend = f1(rank+1, size, N); 
        num_elements = triangular(iend) - triangular(istart); 

        printf("In mm2 : for process %d we have rows of %lu\n", rank, iend - istart);
		...
		for (int i = 0; i < size; i++) {
			/* compute displacement from start of A and B */
                        ...
                        size_t rowstart = f1(i, size, N); size_t rowend = f1(i+1, size, N);
