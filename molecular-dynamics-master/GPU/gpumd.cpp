...
int main() {
	...
	double t0 = omp_get_wtime();	// start the clock (real time)
	int i, j, k;
	const int num_particles = 16000;
	double universe_size[3] = { 128.0, 128.0, 128.0 };	// Position bounds in x, y, z
	...
	double potential_depth = 0.4;
	double VdW_radius = 0.125;
	...
	normal_distribution<> generate_velocity(0.0, 3000.0);
/*	
...
*/
	double positions[num_particles][3];
	double velocities[num_particles][3];
	double accelerations[num_particles][3];
	for (i = 0; i < num_particles; i++) {
		for (j = 0; j < 3; j++) {
	...
	cout << "Generated initial conditions in " << (t1 - t0) << " seconds.\n";
	...
	total_kinetic_energy = 0.0;
                ...
                #pragma omp target teams distribute simd reduction(+:speed_sum,total_kinetic_energy) map(tofrom:positions[0:num_particles][0:3], velocities[0:num_particles][0:3],accelerations[0:num_particles][0:3],speed_sum, total_kinetic_energy) map(to:i,j,k,mass,t_step,universe_size[0:3])
		
                //For each atom, we update its position first then calculate forces and update velocity and accelaration and so on.
			...
			for (j = 0; j < num_particles; j++) {
				if (i != j) {
					...
					if (r < 8.0) {	// cutoff radius
						...
						double Fnorm = LJ_force(r, potential_depth, VdW_radius);
