...
inline double distance(std::vector<double*> positions, int a, int b) { 	// Calculates the straight line distance 
...
inline double speed( std::vector<double*>velocity, int a) { // Calculates the speed of particle a
	double square_speed = 0.0;
	for (int i = 0; i < 3; i++) { square_speed += pow(velocity[a][i], 2); }
	return sqrt(square_speed);
}

inline double kinetic_energy( std::vector<double*>velocity, double m, int a) {
	double square_speed = 0.0;
	for (int i = 0; i < 3; i++) { square_speed += pow(velocity[a][i], 2); }
